        ...
        <div class="relative flex flex-col min-w-0 mb-6 break-words bg-white border-0 shadow-soft-xl rounded-2xl bg-clip-border">
            ...
            <div class="flex-auto p-4">
                <div class="flex justify-center">
                    <p class="text-center">Title lama : <span>{{ $servicestitle }}</span></p>
                </div>
                <form method="POST" action="{{ route('views.services.title') }}" enctype="multipart/form-data">
                    @csrf
                    <div class="flex justify-center">
                        <input type="hidden" name="old_title" value="{{ $servicestitle }}">
    ...
    <div class="flex-none w-full max-w-full px-3 mt-6">
        <div class="relative flex flex-col min-w-0 mb-6 break-words bg-white border-0 shadow-soft-xl rounded-2xl bg-clip-border">
            <div class="p-4 pb-0 mb-0 bg-white rounded-t-2xl">
                ...
                <p class="leading-normal text-sm">Tambahkan atau hapus beberapa services item</p>
            </div>
            <div class="flex-auto p-4">
                @if ($items->count())
                            ...
                            <div class="relative flex flex-col min-w-0 break-words bg-transparent border-0 shadow-none rounded-2xl bg-clip-border">
                                <div class="relative">
                                    <a class="block shadow-xl rounded-2xl">
                                        <img style="margin: auto" src="{{ asset('assets') . '/' . $item->image }}" alt="img-blur-shadow" class="max-w-full shadow-soft-2xl rounded-2xl" />
                                    </a>
                                </div>
                                <div class="flex-auto px-1 pt-6 text-center">
                                    ...
                                    <div class="flex items-center justify-between">
                                        <form style="margin: auto" action="{{ route('views.services.item.delete', $item->id) }}" method="post">
                                            @csrf
                                            @method('delete')
                ...
                @else
                    <div class="flex item-center">
                        <p class="w-full max-w-full text-center text-red-600">Anda belum menambahkan item.</p class="">
                    </div>
                @endif
            </div>
            <div class="p-4 pb-0 mb-0 bg-white item-center rounded-t-2xl">
                <p class="leading-normal text-sm justify-center text-center">Form penambahan Services Item</p>
            </div>
            <div class="flex-auto p-4">
                <div class="flex justify-center mx-3">
                    <div class="w-full max-w-full shadow-soft-2xl justify-center px-3 mb-6 md:w-6/12 md:flex-none xl:mb-0 xl:w-6/12">
                        <div class="relative justify-center flex flex-col h-full min-w-0 break-words bg-transparent border border-solid shadow-none rounded-2xl border-slate-100 bg-clip-border">
                            <form role="form" method="POST" action="{{ route('views.services.item') }}">
                                @csrf
